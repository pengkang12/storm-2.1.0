...
sudo tc qdisc add dev $device parent 1:20 handle 20: netem delay ${delay2}ms
sudo tc qdisc add dev $device parent 1:30 handle 30: netem delay ${delay1}ms
...
sudo tc filter add dev $device parent 1: protocol ip prio 30 u32 match ip dst $edge4 flowid 1:30
sudo tc filter add dev $device parent 1: protocol ip prio 40 u32 match ip dst $edge5 flowid 1:40
