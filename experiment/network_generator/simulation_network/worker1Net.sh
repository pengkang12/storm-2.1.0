...
#sudo tc class delete dev $device classid 1:30
...
sudo tc class add dev $device parent 1:1 classid 1:30 htb rate ${band2}mbit 
sudo tc class add dev $device parent 1:1 classid 1:40 htb rate ${band1}mbit 
sudo tc class add dev $device parent 1:1 classid 1:50 htb rate ${band1}mbit 
...
sudo tc qdisc add dev $device parent 1:30 handle 30: netem delay ${delay2}ms
sudo tc qdisc add dev $device parent 1:40 handle 40: netem delay ${delay1}ms
sudo tc qdisc add dev $device parent 1:50 handle 50: netem delay ${delay1}ms



# Create traffic control filters to match specific IP addresses and assign them to the desired classes
sudo tc filter add dev $device parent 1: protocol ip prio 20 u32 match ip dst $edge3 flowid 1:20
sudo tc filter add dev $device parent 1: protocol ip prio 30 u32 match ip dst $core1 flowid 1:30
sudo tc filter add dev $device parent 1: protocol ip prio 40 u32 match ip dst $edge1 flowid 1:40
sudo tc filter add dev $device parent 1: protocol ip prio 50 u32 match ip dst $edge2 flowid 1:50
